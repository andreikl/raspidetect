...
static int h264_cabac_init_decoder(struct app_state_t* app)
{
    app->h264.cabac.codIOffset = RBSP_READ_UN(&app->h264.rbsp, 9);
    app->h264.cabac.codIRange = 510;

    fprintf(stderr, "INFO: h264_cabac_init_decoder codIOffset (%d)\n", app->h264.cabac.codIOffset);

    return 0;
}

static int h264_cabac_init(struct app_state_t* app)
{
    int res;
    res = h264_cabac_init_context(app);
    if (res) {
        return res;
    }
    res = h264_cabac_init_decoder(app);
    if (res) {
        return res;
    }
    return 0;
...
void h264_RenormD(struct app_state_t* app) {
...
unsigned h264_DecodeDecision(struct app_state_t* app, unsigned ctxIdx)
{
    unsigned binVal;
...
unsigned h264_DecodeBypass(struct app_state_t* app)
{
    int binVal;
    app->h264.cabac.codIOffset <<= 1;
...
unsigned h264_DecodeTerminate(struct app_state_t* app)
{
    int binVal;
...
unsigned h264_DecodeBin(struct app_state_t* app, unsigned ctxIdx, unsigned bypassFlag)
{
    if (bypassFlag) {
        return h264_DecodeBypass(app);
    }
    else {
        if (ctxIdx == 276) {
            return h264_DecodeTerminate(app);
        }
        else {
            return h264_DecodeDecision(app, ctxIdx);
        }
    }
}
...
int h264_get_MbAddr_type(struct app_state_t* app, int* MbAddrN, int xN, int yN, int maxW, int maxH)
{
    ...
    if (xN < 0 && yN < 0) {
        *MbAddrN = h264_MbAddrD(app);
        //fprintf(stderr, "INFO: MbAddrD (%d)\n", *MbAddrN);
    } else if (xN < 0 && yN >= 0 && yN < maxH) {
        *MbAddrN = h264_MbAddrA(app);
        //fprintf(stderr, "INFO: MbAddrA (%d)\n", *MbAddrN);
    } else if (xN >= 0 && xN < maxW && yN < 0) {
        *MbAddrN = h264_MbAddrB(app);
        //fprintf(stderr, "INFO: MbAddrB (%d)\n", *MbAddrN);
    } else if (xN >= 0 && xN < maxW && yN >= 0 && yN < maxH) {
        *MbAddrN = app->h264.data.CurrMbAddr;
        //fprintf(stderr, "INFO: CurrMbAddr (%d)\n", *MbAddrN);
    } else if (xN >= maxW && yN < 0) {
        *MbAddrN = h264_MbAddrC(app);
        //fprintf(stderr, "INFO: MbAddrC (%d)\n", *MbAddrN);
    } else if (xN >= maxW  && yN >= 0 && yN < maxH) {
        *MbAddrN = -1;
        //fprintf(stderr, "INFO: MbAddr: not available\n");
    } else if (yN >= maxH) {
        *MbAddrN = -1;
    } else {
        return -1;
        fprintf(stderr, "ERROR: uncovered case xN(%d) yN(%d)\n%s:%d - %s\n",
            xN, yN, __FILE__, __LINE__, __FUNCTION__);
    }
    return 0;
}

int h264_get_neighbouring_macroblocks(
...
int h264_get_neighbouring_8x8luma(
    struct app_state_t* app,
    int luma8x8BlkIdx, int xD, int yD,
...
int h264_cabac_read_mb_type(struct app_state_t* app)
{
    ...
    GENERAL_CALL(
        h264_cabac_derive_mb_type(app, -1, 0, ctxIdxOffset, &condTermFlagA),
        error
    );
    GENERAL_CALL(
    ...
    if (bin == 0) {
        ...
        if(bin == 1) {
            ...
            if (bin3) {
                ...
                app->h264.data.curr_mb->mb_type_origin +=
                    4 * h264_DecodeDecision(app, ctxIdxOffset + 5);
                //ctxIdxOffset(3): 5 - bin3? 6: 7 (9.3.3.1.2)
                app->h264.data.curr_mb->mb_type_origin +=
                    2 * h264_DecodeDecision(app, ctxIdxOffset + 6);
                app->h264.data.curr_mb->mb_type_origin +=
                    h264_DecodeDecision(app, ctxIdxOffset + 7); //ctxIdxOffset(3): 6 - 7
            }
            else {
                //ctxIdxOffset(3): 4 - bin3? 5: 6 (9.3.3.1.2)
                app->h264.data.curr_mb->mb_type_origin +=
                ...
                //ctxIdxOffset(3): 5 - bin3? 6: 7 (9.3.3.1.2) 
                app->h264.data.curr_mb->mb_type_origin +=
                    h264_DecodeDecision(app, ctxIdxOffset + 7);
            }
            return 0;
        }
    }
...
int h264_cabac_read_transform_size_8x8_flag(struct app_state_t* app)
{
    ...
    int condTermFlagA, condTermFlagB;
    GENERAL_CALL(
        h264_cabac_derive_size_8x8_flag(app, -1, 0, &condTermFlagA),
        error
    );
    GENERAL_CALL(
    ...
    app->h264.data.curr_mb->transform_size_8x8_flag =
        h264_DecodeDecision(app, ctxIdxOffset + ctxIdxInc);
    return 0;
...
int h264_cabac_read_intra4x4_pred_mode(struct app_state_t* app, int i)
{
    ...
    int bin = h264_DecodeDecision(app, 68);
    if (bin) {
        app->h264.data.curr_mb->intra4x4_pred_mode[i] = -1;
        return 0;
    } else {
        // rem_intra4x4_pred_mode
        bin |= h264_DecodeDecision(app, 69);
        bin |= h264_DecodeDecision(app, 69) << 1;
        bin |= h264_DecodeDecision(app, 69) << 1;
...
int h264_cabac_read_intra_chroma_pred_mode(struct app_state_t* app)
{
    ...
    if (!h264_DecodeDecision(app, ctxIdxOffset + ctxIdxInc)) {
        app->h264.data.curr_mb->intra_chroma_pred_mode = 0;
        return 0;
    }
    if (!h264_DecodeDecision(app, ctxIdxOffset + 3)) { // 64: 1 - 3
        app->h264.data.curr_mb->intra_chroma_pred_mode = 1;
        return 0;
    }
    if (!h264_DecodeDecision(app, ctxIdxOffset + 3)) { // 64: 2 - 3
...
int h264_cabac_read_coded_block_pattern(struct app_state_t* app)
{
    GENERAL_CALL(h264_cabac_read_coded_block_pattern_luma(app), error);
    if (app->h264.sps.ChromaArrayType != 0 || app->h264.sps.ChromaArrayType != 3) {
        GENERAL_CALL(h264_cabac_read_coded_block_pattern_chroma(app), error);
    }
...
int h264_cabac_read_mb_qp_delta(struct app_state_t* app)
{
    ...
    struct h264_macroblock_t *prev_mb = h264_is_MbAddr_available(app, app->h264.data.PrevMbAddr)?
    ...
    int bin = h264_DecodeDecision(app, ctxIdxOffset + ctxIdxInc);
    if (bin != 0) {
        bin += h264_DecodeDecision(app, ctxIdxOffset + 2);
    }
    ...
    int blkIdx)
{
    ...
    blocks[blkIdx].coded_block_flag = h264_DecodeDecision(app, ctxIdxOffset + ctxIdxInc);
    return 0;
    ...
    int levelListIdx)
{
    ...
    fprintf(stderr, "ERROR: TO IMPLEMENT ctxBlockCat %d, type %d, %s - %s:%d\n",
    ...
    int levelListIdx)
{
    ...
    coeffLevel->coeff[levelListIdx].coeff_sign_flag = h264_DecodeBypass(app);
    return 0;
}
...
int h264_cabac_end_of_slice_flag(struct app_state_t* app)
{
    ...
    app->h264.data.curr_mb->end_of_slice_flag = h264_DecodeTerminate(app);
    return 0;
}
