...
static void *rfb_function(void* data)
{
    ...
    while (!is_terminated) {
        ...
        if (update_response.message_type != RFBFramebufferUpdateResponse
            ...
            fprintf(stderr,
                "ERROR: RFBFramebufferUpdate response isn't valid: message_type: %d,"
...
#ifdef ENABLE_FFMPEG
        GENERAL_CALL(ffmpeg_decode(app, 4, app->enc_buf_length), error);
#endif //ENABLE_FFMPEG
    }
...
int rfb_init(struct app_state_t* app)
{
    ...
    STANDARD_CALL(pthread_create(&app->rfb.thread, NULL, rfb_function, app), error);
    app->rfb.is_thread = 1;
...
int rfb_handshake(struct app_state_t* app)
{
    ...
    if (app->verbose) {
        fprintf(stderr, "INFO: server_width: %d, server_height: %d, server_name: %s\n",
...
int rfb_start(struct app_state_t* app)
{
    if (sem_post(&app->rfb.semaphore)) {
        fprintf(stderr, "ERROR: sem_post Failed to start rfb with error: %d\n", errno);
        return -1;
    }
    return 0;
}